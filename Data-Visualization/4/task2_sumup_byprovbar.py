...
data = [["updateTime", "provinceName_confirmedTotal"]]
for updateTime, value in data_dict.items():
	row = [updateTime, [["provinceName", "province_confirmedTotal"]]]
	for provinceName, province_confirmedCount in value.items():
		row[1].append([provinceName, province_confirmedCount])
	data.append(row)

with open("task2_sumup_byprovbar.js", "w") as json_output:
        json_output.write("var data = ")

with open("task2_sumup_byprovbar.js", "a") as json_output:
